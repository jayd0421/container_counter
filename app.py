...
with layout_col:
    st.space("medium")
    on = st.toggle('Activate wide layout')
    if on:
        layout = "wide"
    else:
        layout = None
...
user_selected_image_path = None
count_containers = False
...
with subset_tab:
    ...
    with rgb_tiff_file_path_col:
        ...
        if RGB_TIFF_PATH == "":
            pass
        elif RGB_TIFF_PATH.split(".")[-1] != "tif" and RGB_TIFF_PATH.split(".")[-1] != "tiff":
    ...
    with dsm_tiff_file_path_col:
        ...
        if DSM_TIFF_PATH == "":
            pass
        elif DSM_TIFF_PATH.split(".")[-1] != "tif" and DSM_TIFF_PATH.split(".")[-1] != "tiff":
            st.error("Incorrect file extension")
        else:
            file_paths_correct += 1
    ...
    if file_paths_correct == 3:
        ...
        if image_subset_method == 'Tiles':
            if st.toggle("Start container count"):
                count_containers = True
...
with container_count_tab:
    if count_containers:
        # st.html("test_notebbooks/test.html", unsafe_allow_javascript=True)
        ...
        with st.spinner(text="Cleaning container segments...", show_time=True, width="content"):
            ...
            # on = st.toggle("View segmentation")
            # if on:
        ...
        st.warning("Subset your image first.")
